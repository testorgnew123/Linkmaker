...
app.post('/api/forms/contact', async (req, res) => {
  try {
    ...
    if (!profileSlug || !recipientEmail || !formData) return res.status(400).json({ error: 'Missing required fields', code: 'BAD_REQUEST' });
    if (!isValidEmail(recipientEmail)) return res.status(400).json({ error: 'Invalid recipient email', code: 'BAD_REQUEST' });

    const rows = await sql`SELECT u.email FROM profiles p JOIN users u ON u.id = p.owner_id WHERE p.slug = ${profileSlug}`;
    if (rows.length === 0) return res.status(404).json({ error: 'Profile not found', code: 'NOT_FOUND' });
    if (rows[0].email !== recipientEmail) return res.status(403).json({ error: 'Recipient email does not match profile owner', code: 'FORBIDDEN' });
    ...
    const body = [`Name: ${name}`, `Email: ${email}`, phone ? `Phone: ${phone}` : null, message ? `Message:\n${message}` : null].filter(Boolean).join('\n\n');

    await transporter.sendMail({
      from: process.env.SMTP_USER, to: recipientEmail,
      subject: `Contact form submission from ${name || 'visitor'} — ${profileSlug}`, text: body
    });
...
app.post('/api/forms/subscribe', async (req, res) => {
  try {
    ...
    if (!profileSlug || !cardId || !email) return res.status(400).json({ error: 'Missing required fields', code: 'BAD_REQUEST' });
    if (!isValidEmail(email)) return res.status(400).json({ error: 'Invalid email address', code: 'BAD_REQUEST' });

    const profiles = await sql`SELECT slug FROM profiles WHERE slug = ${profileSlug}`;
    if (profiles.length === 0) return res.status(404).json({ error: 'Profile not found', code: 'NOT_FOUND' });
